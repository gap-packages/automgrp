...
##  <ManSection>
##  <Oper Name="IsomorphismPermGroup" Arg="G[, max_lev]"/>
##  <Description>
##  For a given finite group <A>G</A> generated by initial automata or by elements defined by
##  wreath recursion
##  computes an isomorphism from <A>G</A> into a finite permutational group.
##  If <A>G</A> is not known to be self-similar (see <Ref Func="IsSelfSimilar"/>) the isomorphism is based on the
##  regular representation, which works generally much slower. If <A>G</A> is self-similar
##  there is a level of the tree (see <Ref Func="LevelOfFaithfulAction"/>), where <A>G</A> acts faithfully.
##  The corresponding representation is returned in this case. If <A>max_lev</A> is given
##  it finds only the first <A>max_lev</A> quotients by stabilizers and if all of them have
##  different size it returns <K>fail</K>.
##  If <A>G</A> is infinite and <A>max_lev</A> is not specified it will loop forever.
##  <P/>
##  For example, consider a subgroup <M>\langle a, b\rangle</M> of Grigorchuk group.
##  <Example><![CDATA[
...
##  <#GAPDoc Label="Random">
##  <ManSection>
##  <Oper Name="Random" Arg="G"/>
##  <Description>
##  Returns a random element of a group (semigroup) <A>G</A>. The operation is based
