...
################################################################################
##
#F IsAutGroupAbelian . . . . . . . . . Computes if self-similar group is abelian
##    

IsAutGroupAbelian:=function(G)
  local H,a,b,inv,abel;

  H:=AddInverses(G);
  inv:=InverseTable(H);
  abel:=true;

  for a in [2..Length(H[1])] do
    for b in [a+1..Length(H[1])] do
      if not IsOneWord([a,b,a^inv,b^inv],H) then abel:=false; fi;
    od;
  od;
