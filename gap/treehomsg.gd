...
##  is defined as <M>(`PermOnLevelAsMatrix'(g_1)+\cdots+`PermOnLevelAsMatrix'(g_n)+
##  `PermOnLevelAsMatrix'(g_1^{-1})+\cdots+`PermOnLevelAsMatrix'(g_n^{-1}))/(2*n)</M>.
...
##  If the list of <A>weights</A> is given, uses its entries as coefficients of operators correspondings to
